	...
	<section class="content">
		...
		<div class="row">
			<div class="col-xs-12 connectedSortable">
				...
				<button type="button" class="btn btn-primary btn-flat btn-xs" id="jqxGridUnitInsert"><?php echo lang('create'); ?></button>
				<button type="button" class="btn btn-danger btn-flat btn-xs" id="jqxGridUnitFilterClear"><?php echo lang('clear'); ?></button>

				<br /><br />
				<div id="jqxGridUnit"></div>
			</div><!-- /.col -->
		</div>
		<!-- /.row -->
...
<div id="jqxPopupWindow">
   <div class='jqxExpander-custom-div'>
    ...
    <div class="form_fields_area">
        <?php echo form_open('', array('id' =>'form-unit', 'onsubmit' => 'return false')); ?>
            ...
            <table class="form-table">
                ...
                <tr>
                    <th colspan="2">
                        <button type="button" class="btn btn-success btn-xs btn-flat" id="jqxUnitSubmitButton"><?php echo lang('general_save'); ?></button>
                        <button type="button" class="btn btn-default btn-xs btn-flat" id="jqxUnitCancelButton"><?php echo lang('general_cancel'); ?></button>
                    </th>
                </tr>
...
$(function(){
	...
	var unitDataSource =
	{
		...
		url: '<?php echo site_url("admin/unit/json"); ?>',
        ...
        beforeprocessing: function (data) {
        	unitDataSource.totalrecords = data.total;
        },
	    // update the grid and send a request to the server.
	    filter: function () {
	    	$("#jqxGridUnit").jqxGrid('updatebounddata', 'filter');
	    },
	    // update the grid and send a request to the server.
	    sort: function () {
	    	$("#jqxGridUnit").jqxGrid('updatebounddata', 'sort');
	    },
		...
		columns: [
			{ text: 'SN', width: 50, pinned: true, exportable: false,  columntype: 'number', cellclassname: 'jqx-widget-header', renderer: gridColumnsRenderer, cellsrenderer: rownumberRenderer , filterable: false},
			{
				...
				cellsrenderer: function (index) {
					var e = '', d='', row =  $("#jqxGridUnit").jqxGrid('getrowdata', index);
	...
	$("[data-toggle='offcanvas']").click(function(e) {
	    e.preventDefault();
	    $("#jqxGridUnit").jqxGrid('refresh');
	});

	$('#jqxGridUnitFilterClear').on('click', function () { 
		$('#jqxGridUnit').jqxGrid('clearfilters');
	});

	$('#jqxGridUnitInsert').on('click', function(){
    ...
    $("#jqxPopupWindow").jqxWindow({ 
     ...
     $("#jqxUnitCancelButton").on('click', function () {
        $('#id').val('');
        $('#form-unit')[0].reset();
    ...
    $("#jqxUnitSubmitButton").on('click', function () {
        ...
        $('#form-unit').jqxValidator('validate', validationResult);
...
function editRecord(index){
    ...
    var row =  $("#jqxGridUnit").jqxGrid('getrowdata', index);
  	if (row) {
        $('#id').val(row.id);
		$('#name').val(row.name);
...
function deleteRecord(index){
    ...
    var row =  $("#jqxGridUnit").jqxGrid('getrowdata', index);
  	if (row) {
		...
		if (r == true) {
			$.ajax({
                url: "<?php echo site_url('admin/unit/delete_json'); ?>",
                type: 'POST',
                data: {id:[row.id]},
                success: function (result) {
                   $('#jqxGridUnit').jqxGrid('updatebounddata');
                }
            });
		}  
    }
...
function restoreRecord(index){
    ...
    var row =  $("#jqxGridUnit").jqxGrid('getrowdata', index);
  	if (row) {
		...
		if (r == true) {
			$.ajax({
                url: "<?php echo site_url('admin/unit/restore_json'); ?>",
                type: 'POST',
                data: {id:[row.id]},
                success: function (result) {
                   $('#jqxGridUnit').jqxGrid('updatebounddata');
...
function saveRecord(){
    var data = $("#form-unit").serialize();
   
    $.ajax({
        type: "POST",
        url: '<?php echo site_url("admin/unit/save"); ?>',
        data: data,
        success: function (result) {
