...
class Admin extends Rsys_Controller
{
	...
	{
		//$this->db->where('units.delete_flag', 0);
		$this->_get_search_param();
		$total=$this->unit_model->count();
		paging('id');
		//$this->db->where('units.delete_flag', 0);
		$this->_get_search_param();
	...
	public function delete_json()
	{
		...
		if($id && is_array($id))
		{
        	foreach($id as $row):
				...
				$success=$this->unit_model->update('UNITS',$data,array('id'=>$row));
            endforeach;
		}
	}
	...
	public function save()
	{
        ...
        if(!$this->input->post('id'))
        {
            ...
            $success=$this->unit_model->insert('UNITS',$data);
        }
        else
        {
        	$data['modified_by'] = $this->user_id;
        	$data['modified_date'] = date('Y-m-d H:i:s');
            $success=$this->unit_model->update('UNITS',$data,array('id'=>$data['id']));
        }
