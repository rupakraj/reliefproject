...
class Admin extends Rsys_Controller
{
	...
	{
		//$this->db->where('item_types.delete_flag', 0);
		$this->_get_search_param();
		$total=$this->item_type_model->count();
		paging('id');
		//$this->db->where('item_types.delete_flag', 0);
		$this->_get_search_param();
		$rows=$this->item_type_model->getItemTypes()->result_array();
	...
	public function delete_json()
	{
		...
		if($id && is_array($id))
		{
        	foreach($id as $row):
				...
				$success=$this->item_type_model->update('ITEM_TYPES',$data,array('id'=>$row));
            endforeach;
		}
	}
	...
	public function save()
	{
        ...
        if(!$this->input->post('id'))
        {
            ...
            $success=$this->item_type_model->insert('ITEM_TYPES',$data);
        }
        else
        {
        	$data['modified_by'] = $this->user_id;
        	$data['modified_date'] = date('Y-m-d H:i:s');
            $success=$this->item_type_model->update('ITEM_TYPES',$data,array('id'=>$data['id']));
        }
