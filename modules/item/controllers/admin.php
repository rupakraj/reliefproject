...
class Admin extends Rsys_Controller
{
	...
	{
		//$this->db->where('items.delete_flag', 0);
	...
	public function delete_json()
	{
		...
		if($id && is_array($id))
		{
        	foreach($id as $row):
				...
				$success=$this->item_model->update('ITEMS',$data,array('id'=>$row));
            endforeach;
		}
	}
	...
	public function save()
	{
        ...
        if(!$this->input->post('id'))
        {
            ...
            $success=$this->item_model->insert('ITEMS',$data);
        }
        else
        {
        	$data['modified_by'] = $this->user_id;
        	$data['modified_date'] = date('Y-m-d H:i:s');
            $success=$this->item_model->update('ITEMS',$data,array('id'=>$data['id']));
        }
...
$data['expected_till'] = $this->input->post('expected_till');
