...
class Admin extends Rsys_Controller
{
	...
	{
		//$this->db->where('vehicle_types.delete_flag', 0);
		$this->_get_search_param();
		$total=$this->vehicle_type_model->count();
		paging('id');
		//$this->db->where('vehicle_types.delete_flag', 0);
		$this->_get_search_param();
		$rows=$this->vehicle_type_model->getVehicleTypes()->result_array();
	...
	public function delete_json()
	{
		...
		if($id && is_array($id))
		{
        	foreach($id as $row):
				...
				$success=$this->vehicle_type_model->update('VEHICLE_TYPES',$data,array('id'=>$row));
            endforeach;
		}
	}
	...
	public function save()
	{
        ...
        if(!$this->input->post('id'))
        {
            ...
            $success=$this->vehicle_type_model->insert('VEHICLE_TYPES',$data);
        }
        else
        {
        	$data['modified_by'] = $this->user_id;
        	$data['modified_date'] = date('Y-m-d H:i:s');
            $success=$this->vehicle_type_model->update('VEHICLE_TYPES',$data,array('id'=>$data['id']));
        }
