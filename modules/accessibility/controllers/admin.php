...
class Admin extends Rsys_Controller
{
	...
	{
		//$this->db->where('accessibilities.delete_flag', 0);
		$this->_get_search_param();
		$total=$this->accessibility_model->count();
		paging('id');
		//$this->db->where('accessibilities.delete_flag', 0);
		$this->_get_search_param();
		$rows=$this->accessibility_model->getAccessibilities()->result_array();
	...
	public function delete_json()
	{
		...
		if($id && is_array($id))
		{
        	foreach($id as $row):
				...
				$success=$this->accessibility_model->update('ACCESSIBILITIES',$data,array('id'=>$row));
            endforeach;
		}
	}
	...
	public function save()
	{
        ...
        if(!$this->input->post('id'))
        {
            ...
            $success=$this->accessibility_model->insert('ACCESSIBILITIES',$data);
        }
        else
        {
        	$data['modified_by'] = $this->user_id;
        	$data['modified_date'] = date('Y-m-d H:i:s');
            $success=$this->accessibility_model->update('ACCESSIBILITIES',$data,array('id'=>$data['id']));
        }
