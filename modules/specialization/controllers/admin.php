...
class Admin extends Rsys_Controller
{
	...
	{
		//$this->db->where('specializations.delete_flag', 0);
		$this->_get_search_param();
		$total=$this->specialization_model->count();
		paging('id');
		//$this->db->where('specializations.delete_flag', 0);
		$this->_get_search_param();
		$rows=$this->specialization_model->getSpecializations()->result_array();
	...
	public function delete_json()
	{
		...
		if($id && is_array($id))
		{
        	foreach($id as $row):
				...
				$success=$this->specialization_model->update('SPECIALIZATIONS',$data,array('id'=>$row));
            endforeach;
		}
	}
	...
	public function save()
	{
        ...
        if(!$this->input->post('id'))
        {
            ...
            $success=$this->specialization_model->insert('SPECIALIZATIONS',$data);
        }
        else
        {
        	$data['modified_by'] = $this->user_id;
        	$data['modified_date'] = date('Y-m-d H:i:s');
            $success=$this->specialization_model->update('SPECIALIZATIONS',$data,array('id'=>$data['id']));
        }
