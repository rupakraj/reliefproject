...
class Admin extends Rsys_Controller
{
	...
	{
		//$this->db->where('organization_interestareas.delete_flag', 0);
		$this->_get_search_param();
		$total=$this->organization_interestarea_model->count();
		paging('id');
		//$this->db->where('organization_interestareas.delete_flag', 0);
		$this->_get_search_param();
		$rows=$this->organization_interestarea_model->getOrganizationInterestareas()->result_array();
	...
	public function delete_json()
	{
		...
		{
        	foreach($id as $row):
				$success=$this->organization_interestarea_model->delete('ORGANIZATION_INTERESTAREAS',array('id'=>$row));
            endforeach;
		}
	}
	...
	public function restore_json()
	{
		...
		if($id && is_array($id))
		{
        	foreach($id as $row):
				...
				$success=$this->organization_interestarea_model->update('ORGANIZATION_INTERESTAREAS',$data,array('id'=>$row));
            endforeach;
		}
	}
	...
	public function save()
	{
        ...
        if(!$this->input->post('id'))
        {
            ...
            $success=$this->organization_interestarea_model->insert('ORGANIZATION_INTERESTAREAS',$data);
        }
        else
        {
        	$data['modified_by'] = $this->user_id;
        	$data['modified_date'] = date('Y-m-d H:i:s');
            $success=$this->organization_interestarea_model->update('ORGANIZATION_INTERESTAREAS',$data,array('id'=>$data['id']));
        }
