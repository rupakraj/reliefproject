	...
	<section class="content">
		...
		<div class="row">
			<div class="col-xs-12 connectedSortable">
				...
				<button type="button" class="btn btn-primary btn-flat btn-xs" id="jqxGridSkillInsert"><?php echo lang('create'); ?></button>
				<button type="button" class="btn btn-danger btn-flat btn-xs" id="jqxGridSkillFilterClear"><?php echo lang('clear'); ?></button>
...
<div id="jqxPopupWindow">
   <div class='jqxExpander-custom-div'>
    ...
    <div class="form_fields_area">
        <?php echo form_open('', array('id' =>'form-skill', 'onsubmit' => 'return false')); ?>
            ...
            <table class="form-table">
                ...
                <tr>
                    <th colspan="2">
                        <button type="button" class="btn btn-success btn-xs btn-flat" id="jqxSkillSubmitButton"><?php echo lang('general_save'); ?></button>
                        <button type="button" class="btn btn-default btn-xs btn-flat" id="jqxSkillCancelButton"><?php echo lang('general_cancel'); ?></button>
                    </th>
                </tr>
...
$(function(){
    ...
    $("#jqxPopupWindow").jqxWindow({ 
     ...
     $("#jqxSkillCancelButton").on('click', function () {
        $('#id').val('');
        $('#form-skill')[0].reset();
    ...
    $("#jqxSkillSubmitButton").on('click', function () {
        ...
        $('#form-skill').jqxValidator('validate', validationResult);
...
function editRecord(index){
    ...
    var row =  $("#jqxGridSkill").jqxGrid('getrowdata', index);
  	if (row) {
        $('#id').val(row.id);
		$('#name').val(row.name);
...
function deleteRecord(index){
    ...
    var row =  $("#jqxGridSkill").jqxGrid('getrowdata', index);
  	if (row) {
		...
		if (r == true) {
			$.ajax({
                url: "<?php echo site_url('admin/skill/delete_json'); ?>",
                type: 'POST',
                data: {id:[row.id]},
...
function restoreRecord(index){
    ...
    var row =  $("#jqxGridSkill").jqxGrid('getrowdata', index);
  	if (row) {
		...
		if (r == true) {
			$.ajax({
                url: "<?php echo site_url('admin/skill/restore_json'); ?>",
                type: 'POST',
                data: {id:[row.id]},
                success: function (result) {
                   $('#jqxGridSkill').jqxGrid('updatebounddata');
...
function saveRecord(){
    var data = $("#form-skill").serialize();
   
    $.ajax({
        type: "POST",
        url: '<?php echo site_url("admin/skill/save"); ?>',
        data: data,
        success: function (result) {
