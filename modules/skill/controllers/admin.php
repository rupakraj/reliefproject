...
class Admin extends Rsys_Controller
{
	...
	{
		//$this->db->where('skills.delete_flag', 0);
		$this->_get_search_param();
		$total=$this->skill_model->count();
		paging('id');
		//$this->db->where('skills.delete_flag', 0);
		$this->_get_search_param();
		$rows=$this->skill_model->getSkills()->result_array();
	...
	public function delete_json()
	{
		...
		if($id && is_array($id))
		{
        	foreach($id as $row):
				...
				$success=$this->skill_model->update('SKILLS',$data,array('id'=>$row));
            endforeach;
		}
	}
	...
	public function save()
	{
        ...
        if(!$this->input->post('id'))
        {
            ...
            $success=$this->skill_model->insert('SKILLS',$data);
        }
        else
        {
        	$data['modified_by'] = $this->user_id;
        	$data['modified_date'] = date('Y-m-d H:i:s');
            $success=$this->skill_model->update('SKILLS',$data,array('id'=>$data['id']));
        }
