...
class Admin extends Rsys_Controller
{
	...
	{
		//$this->db->where('available_items.delete_flag', 0);
		$this->_get_search_param();
		$total=$this->available_item_model->count();
		paging('id');
		//$this->db->where('available_items.delete_flag', 0);
		$this->_get_search_param();
		$rows=$this->available_item_model->getAvailableItems()->result_array();
	...
	public function delete_json()
	{
		...
		if($id && is_array($id))
		{
        	foreach($id as $row):
				...
				$success=$this->available_item_model->update('AVAILABLE_ITEMS',$data,array('id'=>$row));
            endforeach;
		}
	}
	...
	public function save()
	{
        ...
        if(!$this->input->post('id'))
        {
            ...
            $success=$this->available_item_model->insert('AVAILABLE_ITEMS',$data);
        }
        else
        {
        	$data['modified_by'] = $this->user_id;
        	$data['modified_date'] = date('Y-m-d H:i:s');
            $success=$this->available_item_model->update('AVAILABLE_ITEMS',$data,array('id'=>$data['id']));
        }
...
$data['volunteer_id'] = $this->input->post('volunteer_id');
$data['organization_id'] = $this->input->post('organization_id');
$data['item_id'] = $this->input->post('item_id');
$data['quantity'] = $this->input->post('quantity');
$data['is_recurring'] = $this->input->post('is_recurring');
