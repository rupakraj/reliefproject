...
class Mgenerator extends Admin_Controller
{
	...
	private function _generate_edit_fields($fields)	
	{
		...
		foreach($fields as $row) {
			...
			$type = $row->type;	
			$class = "$('#{$row->name}').val(row.{$row->name});\r\n\t\t";

			if ($type == 'datetime') {
				$class = "$('#{$row->name}').jqxDateTimeInput('setDate', row.{$row->name});\r\n\t\t";
			} elseif ($type == 'date') {
				$class = "$('#{$row->name}').jqxDateTimeInput('setDate', row.{$row->name});\r\n\t\t";
			} elseif (preg_match('/int/is', $type, $matches) && $row->primary_key != '1') {
				$class = "$('#{$row->name}').jqxNumberInput('val', row.{$row->name});\r\n\t\t";
			}
	...
	private function _generate_validation_rules($fields)	
	{
		...
		foreach($fields as $row) {
			...
			$type = $row->type;	
			$rule = "\r\n\t\t\t{ input: '#{$row->name}', message: 'Required', action: 'blur', 
	\t\t\trule: function(input) {
		\t\t\tval = $('#{$row->name}').val();
...
\t\t\t},\r\n";
			...
			if ($type == 'datetime' || $type == 'date') {
				continue;
							...
							$rule = "\r\n\t\t\t{ input: '#{$row->name}', message: 'Required', action: 'blur', 
	\t\t\trule: function(input) {
		\t\t\tval = $('#{$row->name}').jqxNumberInput('val');
		\t\t\treturn (val == '' || val == null || val == 0) ? false: true;
	\t\t\t}
\t\t\t},\r\n";
			}
	...
	private function _generate_column_fields($fields)	
	{
		...
		foreach($fields as $row) {
			...
			$type = $row->type;
			$c = "{ text: '<?php echo lang(\"{$row->name}\"); ?>',datafield: '{$row->name}',width: 150,filterable: true,renderer: gridColumnsRenderer, cellclassname: cellclassname },\r\n\t\t\t";

			if ($type == 'datetime' || $type == 'date') {
				$c = "{ text: '<?php echo lang(\"{$row->name}\"); ?>',datafield: '{$row->name}',width: 150,filterable: true,renderer: gridColumnsRenderer, cellclassname: cellclassname, columntype: 'date', filtertype: 'date', cellsformat:  formatString_yyyy_MM_dd},\r\n\t\t\t";
			}
	...
	private function _generate_form_fields($fields, $primary_key)
	{
		...
		foreach($fields as $row) {
			if ($row->name != $primary_key && !in_array($row->name, $this->discard)) {
						...
						$type = $row->type;	
						$class = "\r\n\t\t\t\t\t<td><input id='{$row->name}' class='text_input' name='{$row->name}'></td>";

						if ($type == 'datetime') {
							$class = "\r\n\t\t\t\t\t<td><div id='{$row->name}' class='date_box' name='{$row->name}'></div></td>";
						} elseif ($type == 'date') {
							$class = "\r\n\t\t\t\t\t<td><div id='{$row->name}' class='date_box' name='{$row->name}'></div></td>";
						} elseif (preg_match('/int/is', $type, $matches)) {
							$class = "\r\n\t\t\t\t\t<td><div id='{$row->name}' class='number_general' name='{$row->name}'></div></td>";
						}

						$cols .= "\r\n\t\t\t\t<tr>";
						$cols .= "\r\n\t\t\t\t\t<td><label for='{$row->name}'><?php echo lang('{$row->name}')?></label></td>";
						$cols .= $class;
						$cols .= "\r\n\t\t\t\t</tr>";
