...
class access_control_model extends MY_Model
{
	...
	function buildACLDropdown($tree_id = NULL,$value_field = 'name', $offsetFlag = true)
	{
		if( $tree_id != 'group' AND $tree_id != 'resource')
		{
			show_error("BackendPro->Access_control_model->buildACLDropDown : The tree_id for the dropdown must be either 'group' OR 'resource'.");
		...
		while($obj->getTreeNext($tree))
		{
			...
			if ($offsetFlag == true){
				...
				if ($tree['row'][$value_field] == 1 || $tree['row'][$value_field] == 2)
					continue;
			}
			$dropdown[$tree['row'][$value_field]] = $offset . $tree['row']['name'];
		}
	...
	{
		// Run Query
		$this->db->select("acl.id, acl.allow, aros.name AS aro, acos.id AS aco_id, acos.name AS aco, axos.name AS axo, actions.axo_id, actions.allow AS axo_allow");
		...
		$this->db->order_by('aro asc, aco, axo, acl.id asc');
	...
	function buildActionSelectorCustomized($aco_id)
	{
		$value = '';
		$q1 = $this->fetch('axos', null,null,array('aco_id'=>null));
		$q2 = $this->fetch('axos', null,null,array('aco_id'=>$aco_id));
		$query = array_merge($q1->result(), $q2->result());
	...
	function getAclActions($resource = null) {

		if ($resource !== null) {
			$this->db->where('acos.name', $resource);
		}

		$this->db->select('axos.id as id, axos.name as `name`, acos.id as aco_id, acos.name as `resource`');
		$this->db->from($this->_TABLES['axos'] . ' axos');
		$this->db->join($this->_TABLES['acos'] . ' acos', 'axos.aco_id = acos.id');
		$q1 = $this->db->get()->result();


		$this->db->select('axos.id as id, axos.name as `name`, "" as aco_id, "" as `resource`', false);
		$this->db->from($this->_TABLES['axos'] . ' axos');
...
/* Location: ./core_modules/auth/models/access_control_model.php */