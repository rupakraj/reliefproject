...
// print $this->db->last_query();
	...
	function form($id = NULL)
	{
		...
		if ($this->validation->run() === FALSE)
		{
			...
			$offsetFlag = ($this->user_id == '1') ? true : false;

			//$data['groups'] = $this->access_control_model->buildAClDropdown('group','id');
			$data['groups'] = $this->access_control_model->buildAClDropdown('group','id', $offsetFlag);	

			$this->load->model('organization/organization_model');
			$this->organization_model->fields = 'organizations.id, organizations.organization_name';
			$orgResults =  $this->organization_model->getOrganizations()->result_array();
			$organizations = array();
			$organizations[0] = 'N/A';
			foreach($orgResults as $org) {
				$organizations[$org['id']] = $org['organization_name'];
			}
