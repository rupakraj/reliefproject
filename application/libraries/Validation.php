...
class Validation  
{
	...
	function spare_vlan($vlan)
	{
		$query = $this->CI->user_model->fetch('UserProfiles',NULL,NULL,array('vlan'=>$vlan));

		// Set the error message
		$this->CI->validation->set_message('spare_vlan', $this->CI->lang->line('userlib_validation_vlan'));

		return ($query->num_rows() == 0) ? TRUE : FALSE;
	}

	function spare_ipaddress($ipaddress)
	{
		$query = $this->CI->user_model->fetch('UserProfiles',NULL,NULL,array('ipaddress'=>$ipaddress));

		// Set the error message
		$this->CI->validation->set_message('spare_vlan', $this->CI->lang->line('userlib_validation_ipaddress'));

		return ($query->num_rows() == 0) ? TRUE : FALSE;
	}

	function spare_edit_vlan($vlan)
	{
		$query = $this->CI->user_model->fetch('UserProfiles',NULL,NULL,array('vlan'=>$vlan,'user_id !='=>$this->CI->input->post('id')));

		// Set the error message
		$this->CI->validation->set_message('spare_edit_vlan', $this->CI->lang->line('userlib_validation_vlan'));

		return ($query->num_rows() == 0) ? TRUE : FALSE;
	}

	function spare_edit_ipaddress($ipaddress)
	{
		$query = $this->CI->user_model->fetch('UserProfiles',NULL,NULL,array('ipaddress'=>$ipaddress,'user_id !='=>$this->CI->input->post('id')));

		// Set the error message
		$this->CI->validation->set_message('spare_edit_ipaddress', $this->CI->lang->line('userlib_validation_ipaddress'));
